...
# Galleries
galleries = ["PG1", "PG2", "S1G1", "S1G2", "S1G3", "S1G4", "S1G5", "S2G1", "S2G2", "S2G3", "S2G4", "S2G5", "S2G6", "S2G7"]

# Provided gallery connections
connections = {
...
def generate_routes_excluding_subsets(graph, entrances, target_galleries):
    routes = set()
    unique_combinations = {}

    def dfs(current, path):
        if current in target_galleries:
            # Generate a key for the current path based on the galleries included, ignoring order
        ...
        for neighbor in graph[current]:
            if neighbor not in path and neighbor in target_galleries:
                dfs(neighbor, path + [neighbor])

    for entrance in entrances:
        dfs(entrance, [entrance])

    # Filter out subsets: keep longer routes when paths include the same set of galleries
...
target_options = [gallery for gallery in galleries if gallery not in entrance_options]
...
# Routes display area
routes_frame = tk.LabelFrame(window, text="Museum walks or routes:")
routes_frame.grid(row=2, column=0, sticky=tk.W, padx=5, pady=5)

routes_label = tk.Label(routes_frame, text="Computer-generated museum walks or routes:")
routes_label.pack(pady=5)

routes_text = tk.Text(routes_frame, width=126, height=10, font=("Arial", 18), wrap=tk.WORD)
routes_text.pack()

def get_selected_entrances():
    """Function to get selected entrances."""
    selected_entrances = []
    for option, var in zip(entrance_options, entrance_vars):
        if var.get():
...
def get_selected_targets():
    """Function to get selected target galleries."""
    selected_targets = []
    for option, var in zip(target_options, target_vars):
        if var.get():
...
def find_routes():
    ...
    if routes:
        ...
        routes_text.insert(tk.END, "No routes found for the selected galleries.\n")
